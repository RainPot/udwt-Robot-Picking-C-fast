...
def resize_img(data, size):
    return data.resize((size[0], size[1]), Image.BILINEAR)


def resize_by_size(data, size):
    img = data[0]
    anno = data[1]
    ori_height, ori_width = img.size[1], img.size[0]
    height, width = size[0], size[1]
    scale_factor_h, scale_factor_w = height/ori_height, width/ori_width

    img = resize_img(img, (height, width))

    anno[:, 0] = anno[:, 0] * scale_factor_w
    anno[:, 1] = anno[:, 1] * scale_factor_h
    anno[:, 2] = anno[:, 2] * scale_factor_w
    anno[:, 3] = anno[:, 3] * scale_factor_h
    return img, anno
...
def whitebalance(data, num):
    if num == 0:
        return data[0], data[1]
    elif num == 1:
        r, g, b = cv2.split(np.array(data[0]))
        B = np.mean(b)
        G = np.mean(g)
        R = np.mean(r)
        K = (R + G + B) / 3
